...
export class ServiceModule {
  ...
  public async addService(url: string) {
    ...
    if (isRemoteUrl) {
      ...
      const fileBuffer: Buffer = fs.readFileSync(resolvedPath);

      return await this._addServiceWithFile(fileBuffer);
    }
  }
  ...
  async changePricingAvailability(
    ...
    if (newAvailability === 'archived' && !fallbackSubscription) {
      throw new Error("You must provide a fallback subscription before archiving. When archiving a pricing version, you must provide a valid subscription in the most recent version of the service's pricing in order to novate all existing contracts to it");
    }

    axios.put(`${this.spaceClient.httpUrl}/services/${serviceName}/pricings/${pricingVersion}?availability=${newAvailability}`, fallbackSubscription, {
      headers: {
   ...
   * 
   * @param file - A buffer containing the file data for the service.
   * @returns A promise that resolves with the response data from the Space API.
   * @throws An error if the operation fails.
   * @private
   */
  private async _addServiceWithFile(file: Buffer): Promise<any> {
    const form = new FormData();
    form.append('pricing', file);

    axios
      .post(`${this.spaceClient.httpUrl}/api/services`, form, {
        headers: {
          ...form.getHeaders(),
          'x-api-key': this.spaceClient.apiKey,
        },
  ...
  private async _addServiceWithUrl(pricingUrl: string): Promise<any> {
    axios
      .post(
        `${this.spaceClient.httpUrl}/api/services`,
        {
          pricing: pricingUrl,
        },
        {
